...
p<-ggplot(data=res, aes(x=reorder(country, -res$`count(id)`), y=res$`count(id)`, colour = reorder(country, -res$`count(id)`), fill = reorder(country, -res$`count(id)`))) +
  geom_bar(stat="identity") + xlab("Country of Residence") + ylab("Number of Climbers")  +scale_color_manual(values=colorPal, aesthetics = c("colour", "fill"), name="Country")
